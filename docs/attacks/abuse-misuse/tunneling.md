...
As preconditions for the attack, the attacker controls a domain such as **attacker.com**, meaning that the attacker has registered the domain and controls its authoritative nameserver, and the victim is already infected with malware capable of accessing sensitive information. In this example, the malware has obtained the password **oi9L\#1**, and the attacker later sends back the instruction **run command.py**. Figure 1 illustrates the tunneling process itself. These are the steps:


- **Step 1:** The malware issues a DNS query for **oi9L\#1.attacker.com**, encoding the stolen password in the subdomain.
- **Step 2:** The DNS resolution process carries that query to the authoritative nameserver for **attacker.com**, where the exfiltrated data becomes available to the attacker.
- **Step 3:** The attacker-controlled nameserver encodes a command such as **run command.py** in a DNS reply, for example in a TXT record.
...
<figure markdown>
  ![Figure 2: GNS3 Lab Topology](../../images/tunneling-1.png){ width="500" }
  <figcaption>Figure 2: Tunneling GNS3 Lab Topology</figcaption>
</figure>
...
| **DNS Server**     |  DNS Server (BIND). Acts as both TLD server and Resolver              | **10.0.4.1**   | 10.0.4.0/24  |
...
On the **DNS Server**,
 ...
 3 - Add or Modify `/etc/bind/db.com` file:
...
```bash
python3 /home/dns_exfiltrator.py
```

The `dns_exfiltrator.py` script on the Attacker Nameserver will capture DNS packets, extract the subdomain, and forward it to the data receiver.
...
The script `tunneling.py` reads a few files like `passwords.txt`, for example, and uses each line as a subdomain for a DNS query, it performs the DNS queries that initiate the tunneling process. Create the files and populate them with a few lines of text.
...
!!! question Question
     The `tunneling.py` script is configured to send DNS queries throught resolvers and not directly to the Attacker Nameserver. Why might this be more effective for an attacker than using direct DNS protocol communication?
...
You can now execute the full attack and observe the results. Ensure the two scripts ( `dns_exfiltrator.py`, `tunneling.py`) are running on their respective machines. Observe the output on each screen.
...
| Node Name  | Role                          | IP Address     | Subnet           |
|------------|-------------------------------|----------------|------------------|
| **Resolver-side IDS9**     | Recursive DNS Server belonging to the Victim’s network             | **eth0: 10.0.4.1 ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ eth1: 10.0.5.254**   | 10.0.4.0/24 10.0.5.0/24 |
| **Victim** | Malware-infected victim. Contains sensitive data          | **10.0.5.1**   | 10.0.5.0/24  |
...
```bash
python3 /home/scapy_attack_blocker.py
```
<br>
